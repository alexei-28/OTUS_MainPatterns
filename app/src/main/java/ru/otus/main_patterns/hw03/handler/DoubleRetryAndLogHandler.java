...
//  Стратегия, если исключение вылетает на FirstTimeCommand, то мы создаём SecondTimeCommand во
// внутрь которой вставляем FirstTimeCommand.
public class DoubleRetryAndLogHandler implements BiFunction<Command, Exception, Command> {
  ...
  public Command apply(Command failCommand, Exception exception) {
    logger.debug(
        "failCommand: {}, exception: {}", failCommand, exception.getClass().getSimpleName());

            ...
            () -> {
              ...
              while (!canStop) { //  Для остановки EventLoop можно сделать отдельную команду.
                ...
                try {
                  command =
                      blockingQueue
                          .take(); // If blockingQueue is empty then thread waiting until a new
                  // command appears in the queue.
                  logger.debug("\neventLoop_execute_command: {}", command);
                  command.execute();
                  logger.debug(
                      "eventLoop_success_executed_command: {} -> take next command", command);
                } catch (Exception ex) {
                  // Если команда выбросила исключение, то оно должно быть обработано только в этой
                  // секции.
                  logger.error(
                      "eventLoop_catch_unsuccess_executed_command: {}, exception: {} -> finding handler",
                      command,
                      ex.getClass().getSimpleName());
                  // Возвращает и выполняет команду, которая вернет систему в работоспособное
                  // состояние.
                  Command successCommand =
                      ExceptionHandler.handle(
                          command, ex); // command - команда, которая выбросила исключение(ex)
                  logger.error(
                      "eventLoop_catch_success_handle_find_successCommand: {} -> execute",
                      successCommand);
                  successCommand.execute(); // Важно, чтобы successCommand.execute() не выбрасывала
                  // исключение.
                  logger.debug(
                      "eventLoop_catch_after_successCommand {} -> Take next command",
                      successCommand);
                }
              }
            });

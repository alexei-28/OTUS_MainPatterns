...
/*-
    ...
    Занятия:
    11. Расширяемая фабрика и IoC + Домашнее задание.
    12. Стратегии разрешения зависимостей IoC
      - более продвинутый вариант расширяемой фабрики (через аннотации + Dependency Injection)
      - таким образом мы избегаем явного вызова метода IoC.resolve(...)

    В идеале, для создания объекта, клиентский код:
     1. НЕ должен:
       - напрямую вызывать "new", e.g. new A()
       - напрямую вызывать методы IoC, e.g. IoC.resolve()

     2. Должен:
       - аннотировать либо конструктор, либо метод используя Dependency Injection

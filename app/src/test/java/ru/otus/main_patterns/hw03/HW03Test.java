...
class HW03Test {
    ...
    private final GetAngleException getAngleException = new GetAngleException("Невозможно получить угол!");
    ...
    void shouldLogWhenThrowException() {
        ...
        // StopCommand специальная команда, которая останавливает(cv.await) thread "EventLoop-thread" в котором работает блокирующая очередь.
        StopCommand stopCommandSpy = spy(new StopCommand());
        // Реализация, которая заменяет логику реального метода(StopCommand#execute).
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            try {
                lock.lock();
                // После выполнения cv.await() "заснет" thread "EventLoop-Thread" и из очереди(в thread-e "EventLoop-Thread") перестанут вытаскиваться команды.
                logger.debug("Executing command: {}\n>>>>>>>>> Current thread goes to sleep >>>>>>>>>", stopCommandSpy);
                cv.await(); // Call in thread "EventLoop-Thread"
                return null;
            } finally {
                lock.unlock();
            }
        }).when(stopCommandSpy).execute();

        RotateCommand rotateCommandMock = mock(RotateCommand.class);
        // При выполнении метода execute команды rotateCommandMock будет выброшено исключение getAngleException
        ...
        try {
            ...
            // После выполнения cv.signalAll() "проснется" thread "EventLoop-Thread" и из очереди (в thread-e "EventLoop-Thread")
    ...
    void handlerShouldAddLogCommandToQueue() {
        ...
        // StopCommand - команда, которая останавливает(cv.await) thread "EventLoop-thread" в котором работает блокирующая очередь.
        StopCommand stopCommandSpy = spy(new StopCommand());
        // Реализация, которая заменяет логику реального метода(StopCommand#execute).
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            try {
    ...
    void shouldRetryCommandThrowException() {
        ...
        // При выполнении метода execute команды rotateCommandMock будет выброшено исключение getAngleException
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing command: {} -> throw exception: {}", rotateCommandMock, getAngleException.getClass().getSimpleName());
            throw getAngleException;
        }).when(rotateCommandMock).execute();

        RetryCommand retryCommandMock = spy(new RetryCommand(rotateCommandMock));
        // При выполнении метода execute команды retryCommandMock будет повторно выброшено исключение getAngleException
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing command: {} -> throw exception: {}", retryCommandMock, getAngleException.getClass().getSimpleName());
            throw getAngleException; // повторитель команды, выбросил исключение
        }).when(retryCommandMock).execute();

        RetryHandler retryHandlerMock = mock(RetryHandler.class);
        // Добавляем в очередь команду RetryCommand,
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing method apply(), handler: {}\nAdd command: {}, successCommand: {}",
                    retryHandlerMock.getClass().getSimpleName(), retryCommandMock, emptyCommand);
            eventLoopService.addCommand(retryCommandMock); // ставим в очередь Команду - повторитель команды, выбросившей исключение
            return emptyCommand; // возвращаем пустую команду
        }).when(retryHandlerMock).apply(any(), any());
    ...
    @Test
    @DisplayName("Обработчик исключения, который ставит в очередь Команду - повторитель команды, выбросившей исключение.")
    void shouldAddRetryCommandThatThrowException() {
        ...
        RetryCommand retryCommandMock = spy(new RetryCommand(rotateCommandMock));
        // При выполнении метода execute команды retryCommandMock будет выброшено исключение getAngleException
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing command: {} -> throw exception: {}", retryCommandMock, getAngleException.getClass().getSimpleName());
            throw getAngleException; // повторитель команды, выбросил исключение
        }).when(retryCommandMock).execute();
    ...
    @Test
    @DisplayName("Обработка исключений - при первом выбросе исключения повторить команду, при повторном выбросе исключения записать информацию в лог")
    void shouldHandleRetryAndLog() {
        ...
        RotateCommand rotateCommandMock = mock(RotateCommand.class);
        // При выполнении метода execute команды rotateCommandMock будет выброшено исключение getAngleException в первый раз
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing command: {} -> throw exception: {}", rotateCommandMock, getAngleException.getClass().getSimpleName());
            throw getAngleException;
        }).when(rotateCommandMock).execute();

        // При повторном выполнении метода execute команды rotateCommandMock будет выброшено исключение getAngleException во второй раз
        RetryCommand retryCommandSpy = spy(new RetryCommand(rotateCommandMock));
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing command: {} -> throw exception: {}", rotateCommandMock, getAngleException.getClass().getSimpleName());
            throw getAngleException;
        }).when(retryCommandSpy).execute();

        RetryHandler retryHandlerMock = mock(RetryHandler.class);
        // Добавляем в очередь команду RetryCommand,
        doAnswer(invocation -> { // Call in thread "EventLoop-thread"
            logger.debug("Executing method apply(), handler: {}\nAdd command: {}, retryCommand: {}",
                    retryHandlerMock.getClass().getSimpleName(), retryCommandSpy, emptyCommand);
            eventLoopService.addCommand(retryCommandSpy); // ставим в очередь Команду - повторитель команды, выбросившей исключение
            return emptyCommand; // возвращаем пустую команду
        }).when(retryHandlerMock).apply(any(), any());
        ...
        // Регистрируем handler-ы
        ExceptionHandler.registerHandler(rotateCommandMock, getAngleException, retryHandlerMock); // при первом выбросе исключения повторить команду
        ExceptionHandler.registerHandler(retryCommandSpy, getAngleException, logHandler); // при втором выбросе исключения записать в лог
    ...
    void shouldFailTwiceAndLog() {
        ...
        // Регистрируем handler-ы
        DoubleRetryAndLogHandler doubleRetryAndLogHandler = new DoubleRetryAndLogHandler(eventLoopService);
        ExceptionHandler.registerHandler(rotateCommandMock, getAngleException, doubleRetryAndLogHandler);
    ...
    */
    private void waitUntilCommandsCountIsZero() {
        await().atMost(TIME_OUT_MS, MILLISECONDS).until(() -> {  // Thread "awaitility-thread"
            logger.debug("Await until the commands count is zero, but not for more than timeout = {} ms.", TIME_OUT_MS);
